...
object ServerSpec extends HttpRunnableSpec {
  ...
  def dynamicAppSpec = suite("DynamicAppSpec") {
    suite("success") {
      testM("status is 200") {
        val status = Http.ok.requestStatus.run()
        assertM(status)(equalTo(Status.OK))
      } +
        testM("status is 200") {
          val res = Http.text("ABC").requestStatus.run()
          assertM(res)(equalTo(Status.OK))
        } +
        testM("content is set") {
          val res = Http.text("ABC").requestBodyAsString.run()
      ...
      suite("not found") {
        val app = Http.empty
        testM("status is 404") {
          val res = app.requestStatus.run()
          assertM(res)(equalTo(Status.NOT_FOUND))
        } +
          testM("header is set") {
            val res = app.requestHeaderValueByName("Content-Length").run()
      ...
      suite("error") {
        ...
        testM("status is 500") {
          ...
          testM("content is set") {
            val res = app.requestBodyAsString.run()
      ...
      suite("echo content") {
        ...
        testM("status is 200") {
          ...
          testM("body is ok") {
            val res = app.requestBodyAsString.run(content = "ABC")
            assertM(res)(equalTo("ABC"))
          } +
          testM("empty string") {
            val res = app.requestBodyAsString.run(content = "")
            assertM(res)(equalTo(""))
          } +
          testM("one char") {
            val res = app.requestBodyAsString.run(content = "1")
      ...
      suite("headers") {
        ...
        testM("headers are set") {
          val res = app.requestHeaderValueByName("Foo").run()
  ...
  def requestSpec = suite("RequestSpec") {
    ...
    testM("has content-length") {
      checkAllM(Gen.alphaNumericString) { string =>
        val res = app.requestBodyAsString.run(content = string)
      ...
      testM("POST Request.getBody") {
        ...
        val res = app.requestStatus.run(!!, Method.POST, "some text")
        assertM(res)(equalTo(Status.OK))
      }
  }
    ...
    testM("data") {
      ...
      testM("status") {
        checkAllM(HttpGen.status) { case (status) =>
          val res = Http.status(status).requestStatus.run()
          assertM(res)(equalTo(status))
        }
      } +
      testM("header") {
        checkAllM(HttpGen.header) { case header @ (name, value) =>
          val res = Http.ok.addHeader(header).requestHeaderValueByName(name).run()
          assertM(res)(isSome(equalTo(value)))
        }
      } +
      testM("text streaming") {
        val res = Http.fromStream(ZStream("a", "b", "c")).requestBodyAsString.run()
        assertM(res)(equalTo("abc"))
      } +
      ...
      testM("file-streaming") {
        ...
        val res  = Http.fromStream(ZStream.fromFile(Paths.get(path))).requestBodyAsString.run()
        assertM(res)(equalTo("abc\nfoo"))
      } +
      suite("html") {
        testM("body") {
          val res = Http.html(html(body(div(id := "foo", "bar")))).requestBodyAsString.run()
          assertM(res)(equalTo("""<!DOCTYPE html><html><body><div id="foo">bar</div></body></html>"""))
        } +
          testM("content-type") {
            val app = Http.html(html(body(div(id := "foo", "bar"))))
            val res = app.requestHeaderValueByName(HeaderNames.contentType).run()
      ...
      suite("content-length") {
        suite("string") {
          testM("unicode text") {
            val res = Http.text("äöü").requestContentLength.run()
            assertM(res)(isSome(equalTo(6L)))
          } +
            testM("already set") {
              val res = Http.text("1234567890").withContentLength(4L).requestContentLength.run()
      ...
      suite("memoize") {
        testM("concurrent") {
          ...
          for {
            ...
            actual   <- ZIO.foreachPar(0 to size)(_ => Http.response(response).requestStatus.run())
          ...
          testM("update after cache") {
            ...
            for {
              ...
              actual <- Http.response(res).withServer(server).requestHeaderValueByName(HeaderNames.server).run()
